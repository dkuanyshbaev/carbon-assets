...
fn deposit_from_set_types_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<Assets as FungiblesCreate<u64>>::create(
            asset1, account1, true, 1
        ));
        assert_ok!(Assets::mint_into(asset1, &account1, 100));

        assert_eq!(First::<Assets>::total_issuance(()), 100);
        assert_eq!(
            First::<Assets>::total_issuance(()),
            Assets::total_issuance(asset1)
        );
