...
fn basic_minting_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Assets::account_balances(1),
            vec![(0, 100), (999, 100), (1, 100)]
        );
    });
}
...
fn minting_insufficient_assets_with_deposit_without_consumer_should_work() {
    new_test_ext().execute_with(|| {
        assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, false, 1));
        assert_noop!(
            Assets::mint(RuntimeOrigin::signed(1), 0, 1, 100),
            TokenError::CannotCreate
        );
        Balances::make_free_balance_be(&1, 100);
...
fn refunding_asset_deposit_with_burn_disallowed_should_fail() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 1, 100));
        assert_noop!(
            Assets::refund(RuntimeOrigin::signed(1), 0, false),
...
fn refunding_with_sufficient_existence_reason_should_fail() {
    new_test_ext().execute_with(|| {
        ...
        // fails to refund
        assert_noop!(
            Assets::refund(RuntimeOrigin::signed(2), 0, true),
...
fn cannot_transfer_more_than_approved() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Assets::transfer_approved(RuntimeOrigin::signed(2), 0, 1, 3, 51),
            e
        );
    });
}
...
fn cannot_transfer_more_than_exists() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Assets::transfer_approved(RuntimeOrigin::signed(2), 0, 1, 3, 101),
            e
        );
    });
}
...
fn force_cancel_approval_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Assets::force_cancel_approval(RuntimeOrigin::signed(2), 0, 1, 2),
            e
        );
        assert_noop!(
...
fn partial_destroy_should_work() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::Assets(crate::Event::Destroyed {
            asset_id: 0,
        }));
...
fn min_balance_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Assets::mint(RuntimeOrigin::signed(1), 0, 2, 9),
            TokenError::BelowMinimum
        );
        assert_noop!(
            Assets::transfer(RuntimeOrigin::signed(1), 0, 2, 9),
            TokenError::BelowMinimum
        );
        assert_noop!(
...
fn transferring_to_blocked_account_should_not_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Assets::block(RuntimeOrigin::signed(1), 0, 1));
        assert_noop!(
            Assets::transfer(RuntimeOrigin::signed(2), 0, 1, 50),
            TokenError::Blocked
        );
        assert_ok!(Assets::thaw(RuntimeOrigin::signed(1), 0, 1));
        assert_ok!(Assets::transfer(RuntimeOrigin::signed(2), 0, 1, 50));
...
fn origin_guards_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Assets::freeze(RuntimeOrigin::signed(2), 0, 1),
            Error::<Test>::NoPermission
        );
        assert_noop!(
            Assets::thaw(RuntimeOrigin::signed(2), 0, 2),
            Error::<Test>::NoPermission
        );
        assert_noop!(
...
fn transferring_from_frozen_account_should_not_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Assets::transfer(RuntimeOrigin::signed(2), 0, 1, 25),
            Error::<Test>::Frozen
        );
        assert_eq!(Assets::balance(0, 1), 50);
...
fn touching_and_freezing_account_with_zero_asset_balance_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Assets::freeze(RuntimeOrigin::signed(1), 0, 2),
...
fn set_metadata_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Assets::clear_metadata(RuntimeOrigin::signed(1), 1),
            Error::<Test>::Unknown
        );
...
fn freezer_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Assets::transfer_approved(RuntimeOrigin::signed(2), 0, 1, 2, 21),
            e
        );
        // a force transfer won't work also.
        let e = Error::<Test>::BalanceLow;
        assert_noop!(
            Assets::force_transfer(RuntimeOrigin::signed(1), 0, 1, 2, 21),
            e
        );
...
#[test]
fn weights_sane() {
    let info = crate::Call::<Test>::create {
        id: 10,
        admin: 4,
        min_balance: 3,
    }
